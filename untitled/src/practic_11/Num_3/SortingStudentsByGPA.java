...
public class SortingStudentsByGPA implements Comparator<Student> {
	...
	public static void main(String[] args) {
		...
		outArray(iDNumber);
		Student[] secArray = setArray(1);
		outArray(mergeSort(iDNumber, secArray));
	}
	public static Student[] setArray(int size) {
		Student[] iDNumber = new Student[size];
		Scanner scan = new Scanner(System.in);
		Random random = new Random();
		System.out.println("Введите имя, фамилию и дату рождения " + size + "студентов");
		for (int i = 0; i < size; i++) {
			iDNumber[i] = new Student(scan.next(), scan.next(), random.nextInt(0, 100),
					random.nextInt(0, 100), random.nextInt(0, 10), scan.next());
		}
		return iDNumber;
	}
